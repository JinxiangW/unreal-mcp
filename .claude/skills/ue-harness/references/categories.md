...
- `unreal_harness_runtime/editor_guard.py`
...
Each domain `server.py` now exports a `TOOLS` list with editor-guarded tools ready for standalone use. Domain servers run independently as separate MCP servers -- connect directly to the domain(s) you need. The orchestrator (`unreal_orchestrator`) exposes only 3 cross-cutting routing/discovery tools. See `config/mcp_config.multi-server.example.json` for a multi-server client configuration.

Key shared utility: `unreal_harness_runtime/editor_guard.py` -- provides `make_guarded_tool()` used by domain servers to wrap live-editor tools with editor readiness preflight. Unguarded tools (harness_info, diagnostics, renderdoc, commandlet) are included in TOOLS as-is.
