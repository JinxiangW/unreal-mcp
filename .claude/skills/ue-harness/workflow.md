...
1. 连接目标域 server（scene / asset / blueprint / material / material_graph / renderdoc / diagnostics）
2. 域 server 内置 editor guard，自动处理 `get_editor_ready_state` / `wait_for_editor_ready`
3. 再执行 guarded 的域命令
4. 不确定目标域时，先连 `unreal_orchestrator` 用 `route_harness_task` 路由
5. `unreal_backend_tcp` raw 工具仅作为 internal / fallback
